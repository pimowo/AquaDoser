// Kolory LED (przykładowe wartości RGB)
#define COLOR_RAINBOW_1 0xFF0000
#define COLOR_RAINBOW_2 0xFF7F00
#define COLOR_RAINBOW_3 0xFFFF00
#define COLOR_RAINBOW_4 0x00FF00
#define COLOR_RAINBOW_5 0x0000FF
#define COLOR_RAINBOW_6 0x4B0082
#define COLOR_OFF 0x000000

// Zmienne globalne
bool serviceMode = false;
unsigned long lastCheckTime = 0;
// Deklaracje funkcji zapisu i domyślnej konfiguracji
void saveConfig();
...
// #include <WebServer.h>         // Serwer HTTP - obsługa strony konfiguracyjnej (ESP32) (usunięto, używamy ESPAsyncWebServer)
...
// Struktura opisująca status pompy
struct PumpStatus {
...
struct PumpSettings {
    ...
    uint8_t pcf8574_pin; // pin PCF8574
};
...
// (usunięto duplikaty include'ów WebServer.h i WebSocketsServer.h)
// WebServer server(80);     // Tworzenie instancji serwera HTTP na porcie 80 (usunięto)
// ESPAsyncWebServer dla ESP32 wymaga AsyncTCP
#include <AsyncTCP.h>
#include <ESPAsyncWebServer.h>
ESPAsyncWebServer asyncServer(80); // Tworzenie instancji asynchronicznego serwera HTTP
...
void setupHA() {
  ...
  device.setModel("AD ESP32");                  // Model urządzenia
  ...
  for (int i = 0; i < NUMBER_OF_PUMPS; i++) {
    ...
    // Usunięto obsługę wskaźników HABinarySensor dla pumpStates
  }
...
void onSoundSwitchCommand(bool state, HASwitch* sender) {
  ...
  AQUA_DEBUG_PRINTF("Zmieniono stan dźwięku na: %s", state ? "WŁĄCZONY" : "WYŁĄCZONY");
}
...
// Obsługa aktualizacji oprogramowania przez HTTP (ESPAsyncWebServer)
